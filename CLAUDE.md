# 公共交通空白地域分析 — Claude Code 向けプロジェクト仕様（125mメッシュ版）

## 概要

「車がないと生活できない地域（公共交通空白地域）」を全国125mメッシュ（L6・2進分割）で可視化し、該当人口を定量化する。
...
| メッシュ解像度 | 250mまたは500m | **125m（L6）固定** |
...
### 分析結果（令和2年国勢調査・実行後に記載予定）

駅スナップ: S12ホーム線形の重心1点 → NE/NW/SE/SW 4象限スナップ
アクセスリンク: 1,000ノード未満の孤立成分を除外（L6アクセスリンク生成後に確定）
...
├── input/
│   └── 2020_pop_census_mesh125.parquet  # 125mメッシュ人口（令和2年国勢調査・KEY_CODE 11桁）
└── output/
    ├── transit_desert.parquet          # 全メッシュ別カテゴリ
    ├── transit_desert.qml              # QGISスタイル（全件用・3色）
    ├── transit_desert_with_pop.parquet # 人口ありメッシュのみ（QGIS推奨）
    ├── transit_desert_with_pop.qml     # QGISスタイル（人口あり用・categorized）
    ├── transit_desert_125m.pmtiles     # ウェブ公開用PMTiles
...
| 本試算（125m版・実行後に記載） | — | — | 道路距離Dijkstra・125mL6（令和2年国勢調査） |
...
### 125mメッシュ人口（令和2年国勢調査）
`input/2020_pop_census_mesh125.parquet` として配置済み
...
# Step 0: L6アクセスリンク生成（初回のみ・nationwide_walk内に出力）
cd ../01_MakeNetwork
...
# Step 5: 人口集計（2020_pop_census_mesh125.parquet 配置済み）
...
| 道路リンク数 | 24,045,959件 |
| 道路ノード数 | 18,614,424件 |
| L6（125m）アクセスリンク数 | 23,740,720件（334MB） |
...
**PMTiles S3配置先**:
```
https://shiworks2.xsrv.jp/pmtiles/mlit-ksj/transit_desert_125m.pmtiles
```
...
| ファイル | QGIS | ウェブ | 集計 |
|---|---|---|---|
| `transit_desert_with_pop.parquet` + `.qml` | **推奨** | — | — |
| `transit_desert.parquet` + `.qml` | 全件 | — | — |
| `transit_desert_125m.pmtiles` | — | S3配置後 | — |
| `summary_national.csv` | — | — | ✓ |
| `pref_ranking.png` / `urban_rural_compare.png` | — | — | ✓ |

## 注意事項

- L6（125m）アクセスリンクは `make_access_links.py --nationwide --level 6 --case nationwide_walk` で生成（L5の約4倍のメッシュ数）
- `02_calc_transit_desert.py` の Dijkstra 処理は全国で約6分（駅140秒 + バス停18秒）
- 125mメッシュ人口データ `2020_pop_census_mesh125.parquet` の秘匿値（`*`）は `03_aggregate.py` で 0 扱い
- `transit_desert_with_pop.parquet` は `pop_total > 0` のみ。全メッシュは `transit_desert.parquet`
